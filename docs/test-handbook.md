...
3. **Pi5 での INGEST / DIST エクスポート**
   ...
   - Pi5 では `sudo ./scripts/server/toolmaster/install_usb_services.sh` を実行して systemd/udev を展開すると、USB 接続時に上記コマンドが自動で起動する（`docs/system/documentviewer-integration.md` 参照）。手動試験でも udev/systemd のステータスを合わせて記録する。
