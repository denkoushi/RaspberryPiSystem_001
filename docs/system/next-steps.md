...
| 体制整備 | 新規 | Pi4 (Window A) を `~/RaspberryPiSystem_001` ベースに刷新し、旧 `~/tool-management-system02` は `*_legacy_` へ退避する（systemd 停止→clone→venv 再構築→toolmgmt.service 差し替え。USB 同期は `window_a/usb_sync.py` のスタブ経由で暫定対応） | docs/system/repo-structure-plan.md:30-58, docs/test-notes/2025-11/window-a-demo.md |
