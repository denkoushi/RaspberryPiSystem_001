...
| コード実装 | 進行中 | Pi5 `/api/v1/production-plan` / `/api/v1/standard-times` のモック実装を追加済み（JSON ファイル or DB テーブル `production_plan_entries` / `standard_time_entries` を選択可）。今後は実データ API へ接続し、Window A Dashboard に本番値を表示させる。 | server/src/raspberrypiserver/api/production.py, server/config/production-plan.sample.json, server/config/standard-times.sample.json, server/config/schema.sql |
