...
| コード実装 | 進行中 | Window A Dashboard の工具管理ペインを Pi5 REST API ベースに刷新。`window_a/scripts/import_tool_master.py`＋ TM-DIST フローは 2025-11-15 に実機確認済みで、API トークン CLI（`scripts/manage_api_tokens.py`）も追加した。次は Pi5 API (`/api/v1/loans` 手動返却等) の実データ連携を仕上げる。 | window_a/app_flask.py, window_a/templates/index.html, docs/system/window-a-toolmgmt.md, server/src/raspberrypiserver/api/tool_management.py |
