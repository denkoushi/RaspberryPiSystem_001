...
| 体制整備 | 進行中 | Pi5/Pi4/Pi Zero のホスト名・systemd・ログ出力をすべて `RaspberryPiSystem_001` へ名寄せ（hostname + `/etc/hosts` + PS1、WorkingDirectory/ExecStart、`/srv|~/RaspberryPiSystem_001/**/logs`。Pi5 のファイルロギング実装は `server/tests/test_logging_config.py` で検証済み） | docs/system/repo-structure-plan.md:42-66, server/config/default.toml:1-22 |
