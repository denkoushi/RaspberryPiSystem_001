...
| コード実装 | 進行中 | Window A Dashboard の工具管理ペインを Pi5 REST API ベースに刷新。`window_a/app_flask.py` では `_proxy_toolmgmt_request()` や UI 用 API トークン表示、操作ログ/自動更新 (`/api/toolmgmt/overview` の AJAX 再描画) まで実装済み。次は Pi4 で TM-DIST→`import_tool_master.py`→Dashboard 操作を実機確認する。 | window_a/app_flask.py, window_a/templates/index.html, docs/system/window-a-toolmgmt.md, server/src/raspberrypiserver/api/tool_management.py |
