...
| コード実装 | 新規 | Pi5 `/api/v1/production-plan` / `/api/v1/standard-times` のモック実装を追加し、JSON ファイルによるサンプル表示を Window A Dashboard で確認できるようにする（2025-11-14 に FileJSONProvider 実装済み）。今後は実データ API へ接続する。 | server/src/raspberrypiserver/api/production.py, server/config/production-plan.sample.json, server/config/standard-times.sample.json |
