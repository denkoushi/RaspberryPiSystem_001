...
| 実機検証 | 進行中 | Pi Zero → Pi5 → Window A 統合テスト（2025-11-14 09:45 Pi Zero 実機 → Pi5 ログ／Window A リスナーの `scan.ingested` 受信まで確認。DocumentViewer ログが 11/13 で停止しているため、service/log 設定を点検し次スキャンで記録する） | docs/test-notes/2025-11/pi-zero-test-plan.md, docs/system/pi-zero-integration.md, docs/test-notes/2025-11/window-a-demo.md |
