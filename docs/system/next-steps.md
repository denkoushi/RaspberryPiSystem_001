...
| コード実装 | 新規 | Pi5 `/api/v1/part-locations` / `/api/logistics/jobs` のデータ源を DB 化し、Window A Dashboard に実データを表示させる。現在は `SCAN_REPOSITORY_BACKEND=memory` のため再起動で所在情報が消える。`/api/logistics/jobs` はプレースホルダー実装済み（空リスト返却）なので、Provider を差し込み実データを返す。PostgreSQL 起動後に `server/config/local.toml` で `db` backend を有効化し、バッチ/Provider を整備する。 | server/config/local.toml, server/src/raspberrypiserver/**, docs/system/postgresql-setup.md |
