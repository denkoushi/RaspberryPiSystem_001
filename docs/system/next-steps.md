...
| コード実装 | 進行中 | Window A Dashboard の工具管理ペインを Pi5 REST API ベースに刷新。TM-DIST → importer → Dashboard → Pi5 `/api/v1/loans` 連携は 2025-11-15 に手動返却/削除ボタンまで確認済み。今後は本番 CSV／DocumentViewer USB／Pi4 NFC リーダー／ハンディリーダ（tools01）との実データ統合を進め、UI 文言（成功時も「エラー」と表示される箇所など）を整える。 | window_a/app_flask.py, window_a/templates/index.html, docs/system/window-a-toolmgmt.md, server/src/raspberrypiserver/api/tool_management.py |
