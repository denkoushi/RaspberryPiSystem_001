...
-  - 同時に `/usr/local/bin/lib/toolmaster-usb.sh` と `/usr/local/bin/tool-{ingest-sync,dist-export,dist-sync,backup-export}.sh` を最新化するため、手動コピーは不要。Pi5 実機では `git pull` → `sudo ./scripts/server/toolmaster/install_usb_services.sh` を実行すれば、systemd/udev と `/usr/local/bin` がすべて最新版になる。  
