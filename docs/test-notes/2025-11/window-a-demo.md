...
- 2025-11-17 13:35: Pi4 側で再度テストしたところ「ブラウザリロード無しでも複数回の貸出が反映される」状態になった。Pi5 `/api/v1/loans` への POST が 200 を返すようになったことで、貸出登録後に `waiting_user` へ戻る挙動が確認できた。ただし、まれに `scan_error` 状態に落ちるケースがあるため、次のテストで連続スキャン 3 セットの安定度を評価予定。
