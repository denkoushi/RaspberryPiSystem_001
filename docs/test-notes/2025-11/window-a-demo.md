...
| 2025-11-13 10:30 | TM-DIST → DocumentViewer USB 同期（Pi5 export + Pi4 importer） | `/srv/RaspberryPiSystem_001/server/logs/usb_dist_export.log`（09:54 `dist export completed`） | `sudo /usr/local/bin/tool-dist-sync.sh --device /dev/sda1`, `sudo DOCVIEWER_HOME=/home/tools02/RaspberryPiSystem_001/document_viewer /usr/local/bin/document-importer.sh /media/tools02/TM-DIST` | `/var/log/document-viewer/import.log` と `client.log` に `Document lookup success: TEST-001` を確認 | 一部手動 | Pi4 の importer サービスは旧 `/usr/local/documents` を参照していたため、DOCVIEWER_HOME を付与した手動実行で同期。サービス側の環境変数更新が今後の TODO。 |
